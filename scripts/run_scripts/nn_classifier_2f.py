...
from src.datasets import AmarettoDataset2F
from src.models import MLP
from src.nn_classifier import MLPClassifierFull, generate_params
from utils.samplers import WeightedSampler


# Add argument parser
def parse_args():
    parser = argparse.ArgumentParser(description='Neural Network Classifier with Full Feature Set')
    parser.add_argument('--seed', type=int, default=None, help='Random seed for reproducibility')
    return parser.parse_args()


### Experiment setup
experiment_name = "amaretto/nn_classifier_2f"
...
if __name__ == "__main__":
    ...
    (hdim, n_layers, activation, mbsize, positive_ratio, dropout), experiment_id = (
        generate_params(args.seed)
    )
    results_dir = RESULTS_DIR / experiment_name / experiment_id
    if os.path.exists(results_dir):
        raise RuntimeError(
            "Could not find correct hyperparameter combination for maximum number of tries. Abort operation."
        )
    else:
    ...
    train_dataset = AmarettoDataset2F(split="train")
    val_dataset = AmarettoDataset2F(split="val")
    # train_dataset = AmarettoDataset2F(split="test")
    ...
    # Models
    mlp = MLP(
        idim=n_prior_features + n_costly_features,
        hdim=hdim,
        n_layers=n_layers,
        dropout=dropout,

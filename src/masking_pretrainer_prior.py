...
from src.dime_utils import generate_uniform_mask
...
class MaskingPretrainerPrior(pl.LightningModule):
    ...
    def __init__(self, model, mask_layer, lr, loss_fn, val_loss_fn, factor=0.2, patience=2, min_lr=1e-6, early_stopping_epochs=None):
        super().__init__()
        ...
        # train and validation epoch end
    ...
    def configure_optimizers(self):
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(opt, factor=self.factor, patience=self.patience, min_lr=self.min_lr, verbose=True)

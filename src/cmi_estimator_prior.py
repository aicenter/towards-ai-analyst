...
from src.dime_utils import get_confidence, get_entropy, ind_to_onehot
...
class CMIEstimatorPrior(pl.LightningModule):
    ...
    def set_stopping_criterion(self, budget=None, lam=None, confidence=None):
        ...
        if budget is not None and confidence is not None:
            self.mode = "confidence_budget"
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Initialize mask with 0 features
        mask = torch.zeros(len(x), self.mask_size, dtype=x.dtype, device=x.device)

        # Initialize the losses for both value and predictor network to be 0
        ...
        for _ in range(self.max_features):
            ...
            # best feature is selected based on predicted CMI and feature cost
            ...
            # generate integer mask with 1s with 1-eps probability
            exploit = (torch.rand(len(x), device=x.device) > self.eps).int()

            # generate the actions based on both best and exploit (which basically adds noise)
            actions = exploit * best + (1 - exploit) * random

            # create new mask based on these actions
    ...
    def on_validation_epoch_end(self):
        pred_list, y_list = zip(*self.validation_step_outputs)
        # y = torch.cat(y_list)
        # preds_cat = [torch.cat(preds) for preds in zip(*pred_list)]
        # pred_loss_ = [self.loss_fn(preds, y).mean() for preds in preds_cat]
        pred_loss_ = [self.loss_fn(preds, y).mean() for preds, y in zip(pred_list, y_list)]
        # val_loss_ = [self.val_loss_fn(preds, y) for preds in preds_cat]
        val_loss_ = [self.val_loss_fn(preds, y) for preds, y in zip(pred_list, y_list)]
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for step in range(self.mask_size):
            ...
            # Compute confidences for the current prediction.
            confidences = get_confidence(pred)

            # Store the intermediate result.
            intermediate_results.append({"step": step, "mask": mask.cpu().clone(), "pred": pred.cpu().clone(), "confidences": confidences.cpu().clone()})
            ...
            if self.mode == "penalized":
                ...
                raise ValueError("Unknown mode in predict_step")

            # Ensure positive CMI.
            accept_sample = torch.bitwise_and(accept_sample, pred_cmi.max(dim=1).values > 0)

            # Stop if no samples are accepted.
            if accept_sample.sum().item() == 0:
                break
        ...
        intermediate_results.append({"step": step + 1, "mask": mask.cpu().clone(), "pred": pred.cpu().clone(), "confidences": confidences.cpu().clone()})

        result = self.format_intermediate_results(intermediate_results)

        if len(batch) == 3:
            return mask.cpu(), pred.cpu(), y.cpu(), result
        else:
            return mask.cpu(), pred.cpu(), result

    def format_predictions(self, outputs, only_intermediate):
        """Format predictions output by trainer.predict()."""
        if only_intermediate:
            _, _, _, results = zip(*outputs)
            probs, conf, queries = zip(*results)

            confidences = np.hstack(conf)
            probabilities = np.hstack(probs)
            queried_features = np.hstack(queries)

            return {"confidences": confidences, "probabilities": probabilities, "queried_features": queried_features}
        ...
        if len(outputs[0]) == 4:
            mask_list, pred_list, y_list, intermediate_results = zip(*outputs)
            mask = torch.cat(mask_list)
            pred = torch.cat(pred_list)
            y = torch.cat(y_list)
            return {"mask": mask, "pred": pred, "y": y, "intermediate": intermediate_results}
        else:
    ...
    @staticmethod
    def format_intermediate_results(batch) -> tuple:
        """
        Function to format probabilities, confidences and queried features for every queried feature.

        Args:
            batch: intermediate result from `predict_step`

        Returns:
            tuple: probabilities, confidences, queried features
        """
        probabilities_dict = {}
        confidences_dict = {}
        feature_set_dict = {}
        num_samples = len(batch[0]["pred"])
        feature_set_dict[0] = [set() for _ in range(num_samples)]

        # part 1
        for n_queried_features, sample in enumerate(batch):
            mask = sample["mask"].numpy()
            probs = nn.Softmax(dim=1)(sample["pred"])[:, 1]
            confidences = sample["confidences"]

            if n_queried_features not in probabilities_dict:
                probabilities_dict[n_queried_features] = probs.numpy()
                confidences_dict[n_queried_features] = confidences.numpy()
            else:
                probabilities_dict[n_queried_features] = np.hstack([probabilities_dict[n_queried_features], probs.numpy()])
                confidences_dict[n_queried_features] = np.hstack([confidences_dict[n_queried_features], confidences.numpy()])

            if n_queried_features == 0:
                continue
            ...
            feature_sets = np.array([set() for _ in range(mask.shape[0])])

            # Create sets of indices using numpy operations
            unique_rows = np.unique(row_indices)
            feature_sets[unique_rows] = [set(col_indices[row_indices == row]) for row in unique_rows]

            if n_queried_features not in feature_set_dict:
                feature_set_dict[n_queried_features] = feature_sets.tolist()
            else:
                feature_set_dict[n_queried_features].extend(feature_sets.tolist())

        # part 2
        probabilities = np.vstack(list(probabilities_dict.values()))
        confidences = np.vstack(list(confidences_dict.values()))
        queried_features = np.zeros(probabilities.shape, dtype=int) - 1

        for ix in range(num_samples):
            # for every sample in the batch (transaction)
            for cf_index in range(1, mask.shape[1]):
                # get the difference at that position with the ones that came before
                union = set().union(*[feature_set_dict[ff][ix] for ff in range(1, cf_index)])
                s = feature_set_dict[cf_index][ix].difference(union)
                # get the item
                item = next(iter(s), -1)
                # add to the matrix
                queried_features[cf_index, ix] = item

        return probabilities, confidences, queried_features
    ...
    def inference(
        ...
        outputs = self.format_predictions(outputs, only_intermediate=only_intermediate)
